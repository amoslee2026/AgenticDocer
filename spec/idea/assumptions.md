...
| A10 | 是否执行 LightRAG 重新摄入 | **【用户指令 2026-09-16：先不要导入 GigaRAG 的文档！！！先开发本项目，再用 GigaRAG 的 markdown 文档做测试】不执行任何 lightRAG 摄入**；`standards/` markdown 先作 AgenticDocer 新系统测试语料；摄入命令当前禁止执行（spec/README 已加 ⛔ 暂缓标注） | 用户明确指令；04_ingested 为空且 LocalServices.md 记录 API key 不一致——摄入前置条件亦未满足 | `ingested_at` 暂缺属预期；ingest.sh 能力（SPEC_SRC_DIR/KEEP_IN_PLACE/DRY_RUN）已就绪待用 | 用户指示后按 spec/README 参考命令执行 |

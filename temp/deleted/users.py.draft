...
字段，绝不写 ``user_id``/``key_fingerprint``（本模块的写入方法均要求 ``actor`` 显式传参，
且由 M06/M07 从**验签结果**取值，不信任客户端自述）。
...
from .rbac import ROLE_PERMISSIONS, authenticate_role, authorize
from .sshsig import fingerprint_of_blob, key_type_for, validate_public_key
...
class _FailureBucket:
    """一个 ``(ip, reason)`` 在当前窗口内的失败计数（进程内，仅用于审计去噪）。"""

    __slots__ = ("bucket", "count", "endpoint", "method", "status_code")

    def __init__(self, bucket: int, endpoint: str | None, method: str | None, status: int) -> None:
        self.bucket = bucket
        self.count = 0
...
def _ssh_key(row: Any) -> SshKey:
    """行 → ``SshKey``；DDL 无 ``fingerprint`` 列，按 ``key_id`` 补齐（M01 模型要求该字段）。"""
...
async def fetch_user_by_username(session: AsyncSession, username: str) -> dict[str, Any] | None:
    row = (
...
async def find_user_by_username(
    username: str, *, db: Database | None = None
) -> User | None:
    async with _db(db).session() as session:
    ...
    *,
    ...
    async with _db(db).transaction() as session:
        ...
        await append_event(
            ...
            op="user_change",
            payload={"user_id": str(record["user_id"]), "action": "create_user",
                     "username": name, "role": role, "status": status},
            actor=actor,
            ts=stamp,
        )
...
async def update_user(
    ...
    *,
    ...
    log.info("user updated", op="update_user", user_id=str(user_id), role=new_role,
             status=new_status)
    return _user({**row, "role": new_role, "status": new_status, "updated_at": stamp})
...
async def delete_user(
    ...
    async with _db(db).transaction() as session:
        ...
        await append_event(
            ...
            entity_id=str(user_id),
            op="user_change",
            payload={"user_id": str(user_id), "action": "delete_user",
...
async def list_ssh_keys(
    ...
    async with _db(db).session() as session:
        rows = (await session.execute(statement)).all()
    return [_ssh_key(row) for row in rows]
...
async def find_active_key(key_id: str, *, db: Database | None = None) -> SshKey | None:
    ...
    async with _db(db).session() as session:
        row = await fetch_active_key(session, normalized)
    return _ssh_key(row) if row is not None else None


async def add_ssh_key(
    user_id: Any, public_key_line: str, *, actor: str, db: Database | None = None
) -> SshKey:
    """登记公钥（校验格式/类型/强度 → 422；同一公钥已属他人 → 409）。"""
    line = " ".join(public_key_line.split())
    try:
        key_type, blob, rsa_bits = validate_public_key(line)
    except SignatureFormatError as exc:
        raise ValidationError(
            f"公钥不合规：{exc.message}", entity="auth", entity_id=str(user_id)
        ) from exc
    key_id = fingerprint_of_blob(blob)
    if key_type == "ssh-rsa" and rsa_bits is not None:
        log.info("RSA 公钥强度低于推荐值", op="add_key", rsa_bits=rsa_bits, key_id=key_id)
    stamp = now()
    async with _db(db).transaction() as session:
        ...
        if existing_row is not None:
            ...
            await session.execute(
                ...
                .values(revoked_at=None, public_key=line, key_type=key_type, added_at=stamp)
            )
            await append_event(
                session,
                entity="auth",
                entity_id=str(user_id),
                op="key_change",
                payload={"user_id": str(user_id), "action": "re_add_key",
                         "key_fingerprint": key_id, "reason": "previously_revoked"},
                actor=actor,
                ts=stamp,
            )
            record = {**existing_row, "revoked_at": None, "public_key": line,
                      "key_type": key_type, "added_at": stamp}
            log.info("ssh key re-added", op="add_key", key_id=key_id, user_id=str(user_id))
            return _ssh_key(record)
        record = {
            "key_id": key_id,
            "user_id": user_id,
            "public_key": line,
        ...
        await append_event(
            ...
            entity_id=str(user_id),
            op="key_change",
            payload={"user_id": str(user_id), "action": "add_key", "key_fingerprint": key_id},
            actor=actor,
            ts=stamp,
        )
    log.info("ssh key added", op="add_key", key_id=key_id, user_id=str(user_id))
    return _ssh_key(record)
...
async def list_grants(
    *, user_id: Any = None, db: Database | None = None
) -> list[Grant]:
    ...
    *,
    ...
    async with _db(db).transaction() as session:
        ...
        if granted_by_id is None:
            granted_by_id = row["user_id"] if isinstance(actor, str) and actor == str(row["user_id"]) else None
        record = {
            "grant_id": new_uuid7(),
            "user_id": user_id,
            "scope": scope,
        ...
        await append_event(
            ...
            entity_id=str(user_id),
            op="grant_change",
            payload={"user_id": str(user_id), "action": "add_grant", "scope": scope,
                     "value": value, "permission": permission},
            actor=actor,
            ts=stamp,
        )
    log.info("grant added", op="add_grant", user_id=str(user_id), scope=scope,
             value=value, permission=permission)
...
async def delete_grant(grant_id: Any, *, actor: str, db: Database | None = None) -> Grant:
    ...
    async with _db(db).transaction() as session:
        ...
        await append_event(
            ...
            entity_id=str(record["user_id"]),
            op="grant_change",
            payload={"user_id": str(record["user_id"]), "action": "remove_grant",
                     "scope": record["scope"], "value": record["value"],
...
async def authorize_user(
    ...
    *,
    ...
    这是 M06/M07 的推荐入口：纯判定逻辑在 ``rbac``（可单测），此处负责 I/O 与审计。
    """
    ...
    try:
        ...
        await log_auth_failure(
            "forbidden",
            status_code=403,
            claimed_user_id=str(user.user_id),
            db=db,
        )
        raise
...
async def log_auth_failure(
    ...
    *,
    ...
    载荷只含**原因**与 ``claimed_*``（自述身份），不含任何密钥材料、不写身份列；
    ``actor='anonymous'``（身份未通过验证）。按 ``(ip, reason, 5min)`` 聚合：
    窗口内首条落事件，后续只计数；换窗时补一条 ``occurrences`` 汇总事件（避免审计淹没）。
    """
    bucket = int(time.time() // _FAILURE_BUCKET_SECONDS)
    ...
    with _failure_lock:
        ...
        if current is not None:
            pending = current if current.count > 1 else None
        entry = _FailureBucket(bucket, endpoint, method, status_code)
        entry.count = 1
    ...
    await _write_failure_event(
        reason,
        ip=ip,
        bucket=_FailureBucket(bucket, endpoint, method, status_code),
        aggregated=False,
        claimed_key_id=claimed_key_id,
        claimed_user_id=claimed_user_id,
        db=db,
    ...
    *,
    ...
    payload: dict[str, Any] = {
        ...
        "bucket_start": datetime.fromtimestamp(
            bucket.bucket * _FAILURE_BUCKET_SECONDS, tz=now().tzinfo
        ).isoformat(),
    }
    if aggregated:
...
async def flush_auth_failure_aggregates(*, db: Database | None = None) -> int:
    ...
    with _failure_lock:
        stale = [(key, entry) for key, entry in _failure_buckets.items() if entry.bucket < current_bucket]
        for key, _ in stale:

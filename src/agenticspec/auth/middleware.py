...
* 失败一律落 ``auth`` 审计事件（S1/S10，:func:`agenticspec.auth.users.log_auth_failure`）
...
from agenticspec.model import Model, User, WriteContext, WriteSource
from agenticspec.observability import DTO_AUTH_REJECTED, get_logger
from agenticspec.store import Database, ForbiddenError, get_database
...
def http_error(exc: Exception) -> HTTPException:
    ...
    if isinstance(exc, AuthError):
        headers = (
            {"WWW-Authenticate": 'SSHSIG realm="agenticspec"'}

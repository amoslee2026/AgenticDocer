...
无编号标题（``section_path`` 为空）按 ADR-006 用「标题 slug + 哈希」。标题自带且与路径
一致的章节号会被剥离，使「整行标题」与「纯标题 + 章节号路径」两种解析形态收敛同一锚。
...
def slugify(title: str) -> str:
    ...
    （如 ``术语与定义`` → ``术语与定义``、``ＡＢＣ`` → ``abc``）。标题自带的一级章节号由
    :func:`anchor_base` 按章节号路径剥离（不在此处做，避免出现第二个归一化口径）。
    """

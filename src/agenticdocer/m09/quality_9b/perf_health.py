...
巡检本体**就是** M12 `health()`（`agenticdocer stats --health` 的同一实现，ADR-010 §3.3：
...
from agenticdocer.observability import HealthReport, health
    ...
    if report.verdict != "ok":
        ...
        violations.append(
            Violation(
                ...
                fix_hint=advice
                or "运行 `agenticdocer stats --health` 查看明细（M12 ADR-010 §3.3）",
            )
        )
    return violations

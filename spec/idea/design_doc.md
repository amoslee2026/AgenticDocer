...
> **arch v1.3/v1.4 回写（2026-09-16）**：检索能力**收窄**——确定性引用检索（多跳遍历）与关键词 FTS 降级为 M05 内部实现，不暴露端点；语义检索由 **LightRAG** 承担（本系统只提供导出包，ADR-008）。详见 `../arch_spec/ADR/ADR-008-M05降级与检索边界.md`。
...
| P4 | 检索与多跳推理 | 检索**归 LightRAG**（本系统经 M-LR 提供导出包）；M05 的 refs 多跳与 FTS 降级为内部实现（ADR-008） | M05（内部）/M-LR |
...
**部署形态**：单机（**可多用户**）、**SSH 公钥签名鉴权（全端点含读）+ WebUI 会话 + RBAC 四角色/文档集级授权**（B6 已作废，ADR-007）、systemd --user（B15）。
...
- **术语校验数据源（E9）**：`terms` 词汇表（随 M09B 引入）——收 glossary 条目与规范性关键词清单（SHALL/SHOULD/MAY/MUST，v0.1 §4.2 RFC2119 风格）；M09B 术语违规 = 违规使用或未定义关键词。

### 5.4 ID 与锚（E7）

- 文档 ID：`SPEC-*`（沿用既有）；节点主键：UUIDv7。
- `anchor`（人类可读 alias）：`<doc_id>#<章节号路径>·<slug(title)>`，如 `SPEC-STD-PCIE-5.0#11.2.3`；**重复标题**（同父同题，实测 CXL「Test Steps:」219 次等）以「**正文摘要** sha256[:8]」消歧、正文亦同者加「同级序号」；**无编号标题**（如「Chapter A2」「Part C Glossary」）用「标题 slug + 哈希」。（修订 A1：原「标题内容摘要」在实测语料上不可互异）
- 稳定性承诺：anchor **仅依赖文档内容与同级计数**（同一源文档重复解析逐字节一致；插入/删除其他块不漂移）；源文档改版重解析时按 (章节号路径, slug) 匹配 + 变更清单人工确认迁移（迁移规则由 M03 实现）。
...
| 规模上限 | 超 B9 量化值（**≥10,000 文档 / ≈13.4M 节点 / 10,000 身份**；B10 已作废） | 触发再评估（分区/归档；ADR-009） |
| LightRAG 迁移 | JSON→PG 迁移与重建策略未定 | 联调前完成（C7 解禁前置条件，§7） |
| 鉴权引入的可用性 | fail-closed 设计下若未自举 admin 则系统不可用 | 自举步骤写入部署清单与 401 错误提示；`auth bootstrap` 可重复救援（ADR-007 S9） |
| 运行期 LLM 依赖 | 若实现中引入 LLM 调用将违背 P6 | 机械验证四项：import lint / 断网 e2e / 依赖树 / 无凭据跑通（REQ-M12-F06） |

## 12. 实施顺序（依赖序，无工期）

```mermaid
flowchart LR
    A["M01 内容模型 + M09A 校验引擎"] --> A2["M10 鉴权与用户管理"]
    A2 --> B["M02 存储"] --> C["M03 导入解析"] --> D["M04 渲染"]
    D --> E["M06 Agent 接口"]
    D --> F["M07 API + M08 前端（并行）"]
    B --> G["M05 图遍历与检索（内部）"]
    D --> H["M09B 质量门"]
    E --> I["M-LR 联调（C7 解禁后）"]
    E --> J["M11 CLI 工具族 + skill"]
    F --> J
    A2 --> K["M12 可观测性（ADR-010）"]
    B --> K
```

1. **地基**：M01（schema 注册表 + 八类原子定义）与 M09A（校验规则库，随 M01 交付）→ **M10（鉴权与用户管理，ADR-007）** → M02（表结构 + 事件日志 + 乐观锁 + 分区，ADR-009）
2. **数据入口**：M03（7 份语料解析→审核→入库；CLI 审核器先行）
3. **出口**：M04（渲染往返，HTML 片段直通；分章节渲染）
4. **人机接口**：M06（agent）与 M07/M08（WebUI，API 契约先行，可并行）
5. **增值**：M05（多跳检索，内部化）、M09B（质量门）、M-LR（联调，前置=LightRAG 存储迁移）
6. **工具与观测**：M11（CLI 工具族 + skill，依赖 M06/M07 契约）、M12（可观测性横切，随 M02 起持续交付）

> **M10 前置理由**（B14）：所有写路径与 API 均需鉴权，先行实现避免全端点返工。M11 随 M10/M06 逐步交付（`auth bootstrap` 属 M10 阶段）。

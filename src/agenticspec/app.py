...
**部署入口**：`agenticspec-api`（pyproject `[project.scripts]`）→ :func:`main`，读
`API_HOST`/`API_PORT`（默认 `127.0.0.1:8787`，§5 安全默认：非 loopback 必须经 TLS 反代）；
也可 `uvicorn agenticspec.app:app`（模块级 :data:`app` 已装配）。
"""
...
from agenticspec import __version__
from agenticspec.agent_api import ErrorResponse
from agenticspec.agent_api import router as agent_router
from agenticspec.auth.router import router as auth_router
from agenticspec.observability import get_logger, install
from agenticspec.store import Database, StoreError, Storage, get_database
...
async def lifespan(application: FastAPI) -> AsyncIterator[None]:
    ...
    purger = asyncio.create_task(_purge_loop(database), name="agenticspec-purge")
...
app = create_app()
"""模块级应用（`uvicorn agenticspec.app:app` 直接可用）。"""


def main(argv: list[str] | None = None) -> int:
    """`agenticspec-api` 入口：`--host`/`--port` 优先，缺省读 `API_HOST`/`API_PORT`。

    `API_HOST` 同时决定 S4（`DEV_MODE` 仅 loopback 开文档面）与 S6（非 loopback 强制 Secure
    Cookie），故命令行的 `--host` 会写回该环境变量，保证两处判定与实际监听一致。
    """
    parser = argparse.ArgumentParser(
        prog="agenticspec-api", description="AgenticSpec API（M06/M07/M10/M12）"
    )

...
role: editor
command: agenticspec import parse|agenticspec import review|agenticspec import commit
---

# spec-import：导入 markdown → 结构化库

## 何时用

- 库内**尚无**该文档（`agenticspec doc list` 里找不到），需要首次导入；
- 源 markdown 大幅改版，需要按新快照重建节点（解析幂等，同锚复用）；
- **不适用**：单点修改已有文档 → 用 `spec-write`；只读/渲染 → `spec-read`/`spec-render`。
...
agenticspec import parse spec/standards/amba/IHI0024_AMBA_APB_spec.md --json

# 2) 审核：逐条处置提议（a 通过 / r 拒绝 / e 修正 / s 跳过 / u 未映射 / A 批量通过非待确认 / q 结束）
agenticspec import review IHI0024_AMBA_APB_spec --accept-confident --json

# 3) 入库：schema 校验 + 事务写入 docs/nodes/refs/events + 资产同步
agenticspec import commit IHI0024_AMBA_APB_spec --json
```
...
| 401/403 | 见下节；403 且提示公钥未注册 → 请 admin `agenticspec user key add` |
...
`spec-annotations`（读人类标注）→ `spec-write`（修订）→ `spec-render` + `spec-diff`（自检）。

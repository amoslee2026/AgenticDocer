"""`python -m agenticdocer.importer`：M03 导入器等价入口（§3 M03：A12 CLI 统一）。

CLI 装配由 M11 统一负责（Typer app / `agenticdocer import`）；本入口只做参数解析与
结果输出，业务逻辑全部在 :mod:`agenticdocer.importer.cli`——两处入口共用同一组业务函数。
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="agenticdocer-import", description="M03 导入解析（parse/review/commit/stats）")

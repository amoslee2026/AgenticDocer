...
:class:`~agenticspec.model.DocTarget` 只带 ``doc_id``；调用方若已解析出文档类型，应经
...
from agenticspec.model import Grant, GrantPermission, GrantTarget, RoleName, User
...
def grant_matches(
    ...
    *,
    ...
    ``grants`` 由调用方提供（通常取 :func:`agenticspec.auth.users.load_grants` 的结果）；
...
def authorize(
    ...
    *,
    ...
    """判定式（S5）：违规抛 :class:`~agenticspec.store.ForbiddenError`（403）。

    纯函数、无 I/O——需要读取 grant 并落审计事件的调用方用
    :func:`agenticspec.auth.users.authorize_user`。
    """
    validate_permission(perm)
...
def validate_grant(role: RoleName, scope: str, value: str, permission: str) -> None:
    ...
    （如给 ``reader`` 授 ``write``）→ :class:`~agenticspec.store.ValidationError`（422）。
    """

...
2. 计时并记录 `route`、`method`、`status`、`dur`。`route` 取**路由模板**
   ...
   （404 未匹配路径、将来在路由前拒答的全局中间件）归入 :data:`UNROUTED` 单一桶，
   原始路径仍写在 ctx 的 `path` 字段供下钻。**注意**：M10 的 401/403 走**依赖解析**、
   此时路由已匹配 → 落**模板桶**（非 `<unrouted>`）；两者刻意区分。
...
装配（`app.py`）：`agenticspec.observability.install(app)`（须最后调用，见下）。
"""
...
from agenticspec.observability.error_codes import DTO_AUTH_REJECTED
from agenticspec.observability.logger import ModuleLogger, get_logger
from agenticspec.observability.rid import new_rid, set_rid
...
#: 路由**匹配前**无法确定路由模板时使用的 `route` 占位值。
#:
#: 典型来源：404（路径不存在）、以及将来在路由前拒答的全局中间件。此时 `scope["route"]`
#: 尚未写入；若回落原始路径，每条被探测路径都会成为独立指标桶（`/a`、`/b`、`/x/y`…）→
#: 基数爆炸且每桶计数为 1，`authFailures` 与错误率退化为无意义数值。故统一归入该桶；
#: 原始路径仍完整保留在 ctx 的 `path` 字段供下钻。
#:
#: M10 的鉴权拒答（`require_auth`/`require_permission` 依赖项）发生在**路由匹配之后**，
#: 落的是**路由模板桶**，不走本占位值。
UNROUTED = "<unrouted>"

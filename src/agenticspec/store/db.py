...
ENV_FILE: Final = Path(
    os.environ.get("AGENTICSPEC_ENV_FILE") or Path(__file__).resolve().parents[3] / ".env"
)
...
DEFAULT_DATABASE_URL: Final = "postgresql+asyncpg://agenticspec_app@127.0.0.1:5432/agenticspec_test"
"""§5/§3 M02 约定的应用连接串（测试库；无密码，凭据由环境变量提供）。"""

DEFAULT_MIGRATION_DATABASE_URL: Final = "postgresql+asyncpg://agenticspec@127.0.0.1:5432/agenticspec"
...
def database_url() -> str:
    """应用连接串（`DATABASE_URL`，最小权限角色 `agenticspec_app`）。"""

...
* **可编辑表格模式（B6/V8）**：``editable = (meta.editable_tables is True OR doc_type ∈
  ...
  :func:`agenticspec.render.editable.write_table_edit`（M07 ``PATCH /nodes/{id}/table``）。
"""
...
from agenticspec.model import Doc, Model, Node, User, UUID7, normalize_body
from agenticspec.store import NotFoundError, as_uuid
    ...
    for node in sorted(nodes, key=_order_key):
        ...
        sections.append(
            SectionInfo(
                ...
                node_count=sizes.get(node.node_id, 0),
            )
        )
    return sections

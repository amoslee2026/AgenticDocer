...
:mod:`agenticspec.agent_api`；本模块只提供「WebUI 专属视图」（列表/diff/schema/术语/批注/
表格编辑/管理端点）。**`/auth/*` 由 M10 提供**（:mod:`agenticspec.auth.router`），此处不重复实现。
...
from agenticspec.model.types import UserStatus
...
from agenticspec.render import TableEdit, resolve_table_mode, write_table_edit
...
from agenticspec.store.comments import COMMENT_COLUMNS
from agenticspec.store.events import EVENT_COLUMNS
...
from agenticspec.agent_api.deps import (
    AdminAuth,
    DatabaseDep,
    ReadAuth,
    ReviewAuth,
...
async def list_comments(
    ...
    """批注列表（M08 批注面板含 `orphaned`；M11 `comment list` / `spec-annotations`）。"""
...
async def doc_diff(
    ...
    to_ts = _resolve_point(doc_events, to, default=None)
...
async def admin_metrics(
    ...
    `log_dir` 走 M12 `snapshot()` 的缺省路径（`logger.log_dir()`：`LOG_DIR`/仓库 `./logs`）。
    """

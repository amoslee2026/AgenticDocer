...
403 复用 M02 的 :class:`~agenticdocer.store.ForbiddenError`（语义与状态码一致，
不引入第二套映射）。
"""
...
from agenticdocer.observability import DTO_AUTH_REJECTED
from agenticdocer.store import StoreError
...
BOOTSTRAP_HINT: Final = (
    ...
    "请先执行 `uv run agenticdocer auth bootstrap`（使用 ADMIN_SSH_PUBKEY_FILE 指向的公钥）完成自举"
)

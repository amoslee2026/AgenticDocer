...
flowchart LR
    ...
    Q2 --> DIAG
    BENCH["tests/perf/bench_*.py<br/>（在线下运行，离线可复现）"] --> DIAG
...
> **P6 说明**：图中「外部 coding agent」为**可选消费者**（虚线），系统运行不依赖其存在；无 LLM 凭据或断网时，M12 的采集/查询/基准全部可用。审计权威源（`events` 表）与运行日志（JSONL）**分离**：日志轮转会丢弃运行痕迹，但审计事件不可丢。

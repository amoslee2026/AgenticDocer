...
本模块是那张表的**机器可读形式**（唯一副本）：frontmatter 推断（:mod:`agenticdocer.importer.
frontmatter`）与测试（`tests/unit/test_doc_type_mapping.py`）都引用它，映射不散落多处。

两条纪律：

1. **`doc_type` 只有 5 值**（`:data:`agenticdocer.model.DOC_TYPES`，与 DDL 的 CHECK 同域）。
...
2. **不在此处定义 schema**（P5 单点）：本模块只做「类型名 → `doc_type`/`doc_subtype`」的
   **分类**；原子 schema 在 :mod:`agenticdocer.model.atoms`，组合规则在
   :mod:`agenticdocer.model.doc_types`。

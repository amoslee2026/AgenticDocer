...
**为什么挑战 nonce 也落 ``nonces`` 表**：它必须服务端留存才可能「一次性 + TTL」，而 DDL
只有这五张鉴权表（无 ``challenges`` 表）。挑战签发是**豁免端点唯一的预认证写库动作**，
受 ``AUTH_RATE_LIMIT_PER_MIN`` 限流；**验签失败不产生任何 nonce 行**（S7 判据）。

**token 熵（S13）**：``secrets.token_urlsafe(32)``（256 位 CSPRNG），库内只存
``SHA256(token)``，明文仅出现在 Cookie 与本次响应体。

**会话失效（S8）**：:func:`resolve_session` **JOIN ``users`` 且 ``status='active'``**；
未命中一律删除该 token 对应行 → 禁用用户/吊销密钥/过期会话的既有请求立即 401。

**清理任务（S15）**：:func:`purge_expired` 在**同一个任务**里清会话与 nonce
（``DELETE WHERE expires_at < now()`` / ``seen_at < now() - nonce_ttl``），并顺带
落失败审计的汇总事件。定时触发由 M06/M07 的启动任务负责调用。
"""
...
from agenticdocer.model import Model, Session, User
...
from .users import _user, fetch_active_key, fetch_user, flush_auth_failure_aggregates
...
SESSION_COOKIE_NAME: Final = "agenticdocer_session"
...
def nonce_ttl_seconds() -> int:
    """nonce 保留期 = ``max(2×时间窗, 600s)``——必须 ≥ 时间窗宽度，否则有重放窗口（S3）。"""
...
async def create_challenge(ip: str | None = None, *, db: Database | None = None) -> Challenge:
    ...
    async with _db(db).transaction() as session:
        await session.execute(
            insert(nonces).values(nonce=nonce, user_id=None, seen_at=issued)
        )
...
async def login(
    ...
    *,
    ...
    失败映射：挑战未知/过期/重放 → 401；公钥未注册或属主被禁用 → 403；
    验签失败 → 401。**验签失败不写任何 nonce 行**（S7），也不消费挑战。
    """
    check_rate_limit(ip)
    database = _db(db)
    ...
    async with database.transaction() as session:
        ...
        if stamp - row.seen_at > timedelta(seconds=CHALLENGE_TTL_SECONDS):
        ...
        try:
            ...
            log.warn("login signature rejected", op="login", key_id=key_id, reason=str(exc))
            raise
    ...
    return LoginResult(
        user=_user(user_row), session=build_model(Session, record), token=token
    )


def _new_session_id() -> Any:
...
async def create_session(
    user_id: Any, *, db: Database | None = None
) -> tuple[Session, str]:
    """直接签发会话（供 M06/M07 的自举/会话续期等路径；正常登录走 :func:`login`）。"""
    stamp = now()
    token = secrets.token_urlsafe(32)
    record = {
        "session_id": _new_session_id(),
    ...
    async with _db(db).transaction() as session:
        await session.execute(insert(sessions).values(**record))
    return build_model(Session, record), token
...
async def resolve_session(token: str, *, db: Database | None = None) -> User | None:
    """token → :class:`~agenticdocer.model.User`（无效/过期/属主禁用 → ``None``）。

    S8：``JOIN users`` 且 ``status='active'``；未命中时删除该 ``token_hash`` 行
    （再试即 401）；命中则**滑动续期**（``last_seen_at``/``expires_at`` 前移 8h）。
    """
    if not token:
        return None
    token_hash = session_token_hash(token)
    ...
    async with database.transaction() as session:
        statement = (
            select(
                ...
                sessions.c.user_id.label("s_user_id"),
                sessions.c.token_hash,
                sessions.c.created_at.label("s_created_at"),
        ...
        expires = stamp + timedelta(seconds=session_ttl_seconds())
        await session.execute(
            update(sessions)
            .where(sessions.c.session_id == session_row["session_id"])
            .values(last_seen_at=stamp, expires_at=expires)
        )
    del session_row["last_seen_at"]
    return _user(user_row)


async def logout(token: str | None = None, *, session_id: Any = None, db: Database | None = None) -> bool:
    ...
    async with database.transaction() as session:
        row = (
            await session.execute(select(sessions.c.user_id).where(condition))
        ).first()
        if row is None:
            return False
    ...
    log.info("session destroyed", op="logout", user_id=str(row.user_id))
    return True
...
async def consume_nonce(
    ...
    PK 冲突即重放 → :class:`AuthenticationError`（401）。调用方须处于写完即提交的事务中。
    """
    try:
        await session.execute(
    ...
    *, db: Database | None = None, moment: datetime | None = None
    ...
    async with database.transaction() as session:
        ...
        stale_nonces = await session.execute(
            delete(nonces).where(
                nonces.c.seen_at < stamp - timedelta(seconds=nonce_ttl_seconds())
            )
        )
    report = PurgeReport(

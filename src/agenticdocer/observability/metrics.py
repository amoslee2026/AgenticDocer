...
下的 JSONL 原始日志并聚合——不引入 Prometheus/时序库（ADR-010 明确否决）。

**日志契约（本层与埋点方的接口）**

| 指标 | 采集点写入 | 本层的识别方式 |
|---|---|---|
| 端点耗时/错误率 | `m12.middleware`（HTTP）、`m06/m07.router` | ctx 含 `route`；`status >= 400` 计入错误率 |
...
| 渲染耗时 | M04 | ctx `op ∈ {render_section, render_document}` |

百分位用 **nearest-rank** 法（确定性、无需 numpy）：`k = ceil(q/100 × n)`，取排序后第 k 个。
"""
...
from agenticdocer.observability.logger import PROGRAM, slow_query_ms
...
def _default_log_dir() -> Path:
    from agenticdocer.observability.logger import log_dir
...
def _render_metric(entries: Iterable[dict[str, Any]]) -> RenderMetric:
    ...
    for entry in entries:
        ...
        if dur is None:
            continue
        if op == "render_section":
            sections.append(dur)
        elif op == "render_document":

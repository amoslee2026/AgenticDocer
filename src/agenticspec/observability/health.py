"""容量健康巡检（ADR-010 §3.3、REQ-M12-F05）——`agenticspec stats --health`。
...
from agenticspec.observability.error_codes import DTO_PARTITION_MISSING
...
class HealthReport(_CamelModel):
    """巡检报告（`agenticspec stats --health` 与 M09B `perf_health` detector 共用）。"""

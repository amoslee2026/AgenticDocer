...
* ``nodes.jsonl``——每行 ``{node_id, doc_id, anchor, text}``（§3 M-LR 规定的 jsonl 形状）。
  ``text`` 取 **M04 渲染口径**（:func:`~agenticspec.render.node_block_text`，唯一节点渲染
...
* ``origin='traverse'``：M05 :func:`~agenticspec.retrieve.traverse` 的多跳可达集合
...
from agenticspec.model import Doc, ExportResult, Node
from agenticspec.observability import get_logger
from agenticspec.render import node_block_text
from agenticspec.retrieve import KIND_RULES, traverse

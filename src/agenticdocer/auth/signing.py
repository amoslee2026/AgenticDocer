...
def public_key_line(key: Any) -> str:
    ...
    public = key.public_key() if isinstance(key, (ed25519.Ed25519PrivateKey, rsa.RSAPrivateKey)) else key
...
def key_type_for(public_key_line: str) -> str:
    """公钥行 → ``ssh_keys.key_type`` 取值（DDL 取值域：ed25519 或 rsa-sha2-512/256）。"""
    key_type, _ = key_blob_from_line(public_key_line)
...
def build_sshsig(
    ...
    *,
    ...
    """生成 SSHSIG **raw blob**（Ed25519 直签；RSA 用 PSS + 同名摘要）。"""
    import struct

    public = private_key.public_key()
    line = public_key_line(public)
    key_type, blob = key_blob_from_line(line)
    if key_type == "ssh-ed25519":
        signature_algorithm = "ssh-ed25519"
        signature = private_key.sign(_signed_data(namespace, hash_algorithm, message))
    elif key_type == "ssh-rsa":
        hash_cls = _hash_class(hash_algorithm)
        signature_algorithm = f"rsa-sha2-{hash_cls.digest_size * 8}"
        signature = private_key.sign(
            _signed_data(namespace, hash_algorithm, message),
            __import__(
                "cryptography.hazmat.primitives.asymmetric.padding", fromlist=["PSS"]
            ).PSS(
                mgf=__import__(
                    "cryptography.hazmat.primitives.asymmetric.padding", fromlist=["MGF1"]
                ).MGF1(hash_cls()),
        ...
        raise SigningError(f"不支持的私钥类型：{key_type}")

    def _s(value: bytes) -> bytes:
        return struct.pack(">I", len(value)) + value

    signature_field = _s(signature_algorithm.encode("ascii")) + _s(signature)
    return (
        b"SSHSIG"
        + struct.pack(">I", 1)
        + _s(blob)
        + _s(namespace.encode("utf-8"))
        + _s(b"")
        + _s(hash_algorithm.encode("ascii"))
        + _s(signature_field)
    )


def _hash_class(name: str) -> Any:
    if name == "sha512":
        return hashes.SHA512
    if name == "sha256":
        return hashes.SHA256
    raise SigningError(f"不支持的哈希算法：{name}")
...
def sign_message(
    ...
    *,
    ...
    """签名并编码：默认**单行 base64**（可直接放进 HTTP 头 / JSON），``armored=True`` 出 armor。"""
    blob = build_sshsig(key, message, namespace=namespace, hash_algorithm=hash_algorithm)
    return armor(blob) if armored else base64.b64encode(blob).decode("ascii")


def sign_message_armored(key: Any, message: bytes, **kwargs: Any) -> str:
    """armor 形态（供「上传一次性签名文件」路径与人工核对）。"""
    kwargs["armored"] = True
    return sign_message(key, message, **kwargs)
...
def login_payload(nonce: str) -> bytes:
    """登录签名载荷 = **nonce 的原样 ASCII 字节**（无换行；`auth sign --login` 与登录页共用）。"""

...
* :func:`~agenticdocer.mlr.export.export_package`——生成导出包（渲染文本 + ``node_id``
  映射 + 图结构上下文），**不依赖 lightRAG 在线**（REQ-MLR-F01）；
* :func:`~agenticdocer.mlr.stream.change_stream`——基于 ``events`` 的增量游标流
  （REQ-MLR-F02），:func:`~agenticdocer.mlr.stream.cursor_token` 给出可持久化的续拉游标。

"""M04 渲染 CLI：``python -m agenticspec.render <doc_id> [--section <anchor>]``（§3 M04 / A12）。

用法::

    python -m agenticspec.render SPEC-STD-AMBA-APB              # 整档 → <out>/SPEC-….md
    python -m agenticspec.render SPEC-STD-AMBA-APB --sections   # 章节清单（anchor/title/level）
    python -m agenticspec.render SPEC-STD-AMBA-APB --section 'SPEC…#3·术语'
    python -m agenticspec.render SPEC-STD-AMBA-APB --out-dir build/rendered

呈现层约定：stdout 只承载用户可见结果（路径/清单/错误）；运行日志一律经 ``m04.*`` logger
（ADR-010 禁止业务代码 ``print``/``logging``）。退出码：0 成功、1 文档/章节不存在。

对应 M11 的 console script 名 ``agenticspec-render``（§3 M04）；本模块是等价入口。
"""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m agenticspec.render",
...
def main(argv: list[str] | None = None) -> int:
    """CLI 入口：``python -m agenticspec.render …``；返回进程退出码。"""

...
role: reader
command: agenticspec render
---

# spec-render：渲染为 Markdown
...
```bash
agenticspec render SPEC-STD-AMBA-APB --json                  # 整档（服务端落 build/rendered/）
agenticspec render SPEC-STD-AMBA-APB --section 'SPEC-STD-AMBA-APB#3.2' --json
agenticspec render SPEC-STD-AMBA-APB --markdown               # markdown 直接打到 stdout
agenticspec render SPEC-STD-AMBA-APB --out build/local.md     # 另存一份到本地路径
```

章节锚从 `GET /api/v1/docs/{doc_id}/sections`（`spec-read` 的 sections 清单口径）取得。
...
| 现象 | 处置 |
|---|---|
| 404 | doc_id 不存在 → `agenticspec doc list` 复核 |

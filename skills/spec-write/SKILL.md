---
name: spec-write
description: 需要新增/修改/软删结构化节点时使用（乐观锁写入，409 冲突需重读后重试）。写入前先 spec-diff 感知他方改动，写后 spec-render 自检产物。
role: editor
command: agenticspec node put|agenticspec node delete
---
...
- 按人类标注（`spec-annotations`）或评审意见修订既有节点；
- 补充/修订定义、条款、表格等原子；
...
agenticspec node put --file patch.json --json

# 内联新建（最短路径）
agenticspec node put --doc SPEC-STD-AMBA-APB --atom-type clause --anchor 'SPEC-STD-AMBA-APB#3.2.1·transfer' \
  --content '{"text":"…"}' --json

# 软删（必须给当前 version）
agenticspec node delete <node_id> --expected-version 3 --json
```

`patch.json` 示例（字段口径见 `agenticspec node get --json` 输出）：
...
| 现象 | 处置 |
|---|---|
| **409** | 乐观锁冲突：`agenticspec node get <node_id>` 重取 `version`（并 `spec-diff` 看谁改了什么）后重试；**不要**盲目重放旧载荷 |
| **422** | schema 校验失败：按 `detail.violations[].fixHint` 修正 `content`（八类原子 schema 见 M01） |
| 404 | 节点已软删或 doc_id/anchor 错 → 转 `spec-read` 复核 |
| 403 | 见下节（需 editor） |

## 权限不足补救

```
agenticspec user role --username <你的用户名> --role editor
...
- 修改后必跑 `spec-render`（自检往返一致）+ `spec-diff`（确认改动范围符合预期）。

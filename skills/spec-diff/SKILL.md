...
- **写前必查**：确认自己手上的 `version`/内容没有过期（时序纪律：`spec-diff` → `spec-write`）；
...
```bash
agenticspec doc diff SPEC-STD-AMBA-APB --json                        # 当前 vs 上一次变更（缺省区间）
agenticspec doc diff SPEC-STD-AMBA-APB --from 2026-09-01 --json      # 指定时间区间
agenticspec doc diff SPEC-STD-AMBA-APB --from 12 --to 18 --json      # 指定版本区间
```
...
| 404 | doc_id 不存在 → `agenticspec doc list` |
| 403 | 见下节（需 reader） |
...
`spec-diff`（看别人改了什么）→ `spec-read`（取最新 `version`）→ `spec-write`（写）→ `spec-diff`（确认改动范围）。

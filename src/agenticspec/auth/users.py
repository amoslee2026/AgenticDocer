...
from agenticspec.model import Grant, GrantScope, SshKey, User, new_uuid7
...
from agenticspec.store.rows import as_uuid, row_to_dict
from agenticspec.store.schema import grants, ssh_keys, users
...
def user_from_row(row: Any) -> User:
    """``users`` 行 → :class:`~agenticspec.model.User`。"""
    return build_model(User, _mapping(row))


def ssh_key_from_row(row: Any) -> SshKey:
    """``ssh_keys`` 行 → :class:`~agenticspec.model.SshKey`。
...
def grant_from_row(row: Any) -> Grant:
    """``grants`` 行 → :class:`~agenticspec.model.Grant`。"""
...
async def authorize_user(
    ...
    *,
    ...
    """读取 grant → 判定（:func:`agenticspec.auth.rbac.authorize`）→ 违规落审计事件。

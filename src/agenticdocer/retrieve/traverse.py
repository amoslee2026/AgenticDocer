...
arch/functional spec 只给「上游/下游/双向」文字，未定义箭头）：
...
第 ``step`` 跳允许跟随的 kind = `expandable_kinds(step)`：``step <= max_hops`` 且
...
- 默认过滤 `status='deleted'` 节点：软删节点**既不返回也不再展开**（DFS 语义上视为
  已从图中摘除）；`include_deleted=True` 时二者一并放开。
- `dst_node_id IS NULL` 的**文档级引用**与指向不存在节点的悬空边都不产生命中：
  前者无节点可落，后者由 `nodes` 解析时被丢弃（这正是 M09B `broken_refs` 的口径）。

分区策略（ADR-009 V16）
-----------------------

`nodes` 按 `doc_id` HASH 64 分区，按 `node_id` 单列点查无法裁剪。遍历时**能带
`doc_id` 就带**：

- 根节点：调用方给 `doc_id=` 即裁剪到单分区（缺省则 64 分区 Append）；
- 前沿节点恒持有 `doc_id`（由上一跳解析得到），故 `refs.src_node_id`（down）与
  `refs.dst_doc_id`（up，`idx_refs_dst`）两侧都可裁剪；
- 命中节点的元数据解析按 `doc_id` 分组发查询：down 方向由 `refs.dst_doc_id` 给出
  `doc_id`（裁剪），up 方向只能拿 `node_id` 裸查（不可裁剪，计入 `unpruned` 计数并
  落 `info` 日志）。
...
- 起点自身**不**出现在结果里（`hops >= 1`）；同跳多路径命中时取确定序最前者
  （前沿序 → kind 序 → 目标 doc_id → 目标 node_id），保证同一图两次调用结果一致；
- 纯查询、无事件、无 LLM（P6）。
"""
...
from agenticdocer.model import RefKind, TraversalHit
from agenticdocer.observability import get_logger, slow_query_ms
from agenticdocer.store import NotFoundError, Storage, ValidationError, get_storage
from agenticdocer.store.schema import nodes, refs
...
@dataclass(frozen=True, slots=True)
class _Candidate:
    """一步扩展的候选目标：目标节点 + 已知 `doc_id`（up 方向未知）+ 来源 + kind 序。"""
...
#: 单步扩展取边：`(前沿节点 id, 其 doc_id)` → `(来源节点 id, 目标节点 id, 目标 doc_id|None)`。
FetchEdges = Callable[
...
async def _walk(
    ...
    *,
    ...
    """纯 BFS 核心（可注入取边/解析函数，故无需 PG 即可验证跳数/去重/环）。
    ...
    for step in range(1, hops + 1):
        ...
        for kind_index, kind in enumerate(kinds):
            ...
            if direction in ("down", "both"):
                edges += await fetch_edges(sources, kind)
            if direction in ("up", "both"):
                # up 方向的取边由同一个 fetch_edges 按 kind 内部决定方向，故此处仅透传 kind；
                # `both` 的两侧集合由 fetch_edges 自身合并（见 _fetch_edges）。
                edges += await fetch_edges(sources, kind, reverse=True)  # type: ignore[call-arg]
...
async def _fetch_edges(
    ...
    *,
    ...
    """取一步边（`reverse=False` = src→dst，`True` = dst→src；`both` 由调用方两侧各取一次）。

    - `src→dst`：`idx_refs_src`（ADR-009 新增），目标 doc 由 `refs.dst_doc_id` 给出；
    - `dst→src`：`idx_refs_dst(src=doc_id 裁剪)`，目标 doc 未知（返回 `None`）。
    """
    if not sources:
        return []
    ...
    if reverse:
        ...
        for doc_id, node_ids in grouped.items():
            ...
            for dst_node_id, src_node_id in (await session.execute(statement)).all():
                rows.append((dst_node_id, src_node_id, None))
        return rows
    ...
    rows.extend((await session.execute(statement)).all())  # type: ignore[arg-type]
    return rows
...
async def traverse(
    ...
    *,
    ...
    参数：
        ...
        doc_id: 起点的 `doc_id`（ADR-009 V16 分区裁剪；缺失则全分区探测）。
    ...
    with log.timer("traverse", budget_ms=slow_query_ms(), node_id=str(node_id), hops=hops):
        async with store.db.session() as session:
            roots = await _resolve_nodes(session, ((_as_uuid(node_id), doc_id),), include_deleted)
            if not roots:
                raise NotFoundError(
            ...
            async def fetch_edges(
                sources: Sequence[tuple[UUID, str]], kind: RefKind, *, reverse: bool = False
            ) -> list[tuple[UUID, UUID, str | None]]:
                return await _fetch_edges(session, sources, kind, reverse=reverse)

...
async def test_traces_to_traverses_two_hops_upstream() -> None:
    ...
    # hops 超过 MAX_HOPS 无额外效果（链只有 2 跳）
...
async def test_see_also_is_bidirectional_single_hop() -> None:
    ...
    down = await walked(graph, m, 1)
    assert [hit.node_id for hit in down] == [n]
    up = await walked(graph, n, 1)
    assert sorted(hit.node_id for hit in up) == sorted([m, deeper])
    # 1 跳上限：hops=2 不再延伸
    assert sorted(hit.node_id for hit in await walked(graph, m, 2)) == sorted([n, deeper])
...
async def test_self_loop_yields_nothing() -> None:
    ...
    assert [(hit.node_id, hit.hops) for hit in again] == [(hit.node_id, hit.hops) for hit in hits]
...
async def test_ordering_is_hops_then_doc_then_ordinal() -> None:
    ...
    graph.ref("see_also", hub, b5)
    graph.ref("see_also", hub, a9)
    graph.ref("see_also", hub, a2)
    graph.ref("see_also", hub, far)
    graph.ref("traces_to", far, graph.node(4, doc_id="DOC-A"))  # 2 跳命中：doc 序最前也排最后

...
* `program="agenticspec"`，`command=<模块族>`（`model`/`store`/`api`/…）——文件名
  `agenticspec_<command>.jsonl`，同族模块共用一份日志文件。
* `module` 字段 = `get_logger()` 传入的 `M##.子域`（如 `m02.nodes`），逐行写入。
* **per-entry rid**：每行日志的 `rid` 取自 :func:`~agenticspec.observability.rid.current_rid`
  （ContextVar），而非 SDK 的 run rid——SDK 允许调用方在 auto-fill 前覆盖任意自动字段，
  这是请求级追踪（`agentic-logger trace --rid`）的实现方式；未绑定 rid 时回落 run rid。
* 自定义字段统一进 `ctx`（嵌套 dict，`agentic-logger query --keyword` 可检索）：
  `doc_id` / `op` / `route` / `method` / `status` / `sql_hash` / `table` 等。
  保留名 `dur` / `error_code` / `tid` 由本层提升为顶层字段，不可用作 ctx 键。
...
from agenticspec.observability.error_codes import DTO_PERF_EXCEEDED
from agenticspec.observability.rid import current_rid
...
#: 耗时预算（§1.4 指标）→ `(env 覆盖变量, 默认 ms)`。
#:
#: op 名既是 `timer()` 的分组键，也是 `DTO_PERF_EXCEEDED` 的判定依据：
...
#: 日志保留名：由本层提升为顶层字段，不作为 ctx 键。
_RESERVED_FIELDS = ("dur", "error_code", "tid")

_BACKENDS: dict[tuple[str, str], AgentLogger] = {}
...
def classify_duration(dur_ms: int, budget_ms: int | None) -> tuple[str, str | None]:
    ...
    * 无预算 → INFO；
    * `dur > 预算` → ERROR + `DTO_PERF_EXCEEDED`；
    * `dur > 0.8 × 预算` → WARN（整数比较 `dur*5 > budget*4`，避免浮点误差）；
    * 其余 → INFO。
    """
...
class ModuleLogger:
    ...
    视图对象极轻（无文件句柄、无 IO），可随手 `get_logger(...)`；需要固定上下文字段
    时用 :meth:`child`。
    """
    ...
    def __init__(
        self, module: str, backend: AgentLogger, ctx: Mapping[str, Any] | None = None
    ) -> None:
        self._module = module
    ...
    def child(self, **ctx: Any) -> ModuleLogger:
        """派生一个绑定额外上下文字段的视图（如 `doc_id`），同名键覆盖。"""
    ...
    def debug(self, msg: str, /, **fields: Any) -> None:
        ...
        `AgentLogger` 没有公开的 `debug()`（ADR-010 伪代码声称有），本层经 SDK 的
        `_write(level="DEBUG", ...)` 落盘——该级别在 SDK 的 `_LEVELS` 与查询层均受支持。
        """
    ...
    def exception(self, msg: str, exc: BaseException | None = None, /, **fields: Any) -> None:
        ...
        显式传 *exc*；省略时取 `sys.exc_info()` 的当前异常（须处于 `except` 块内，否则
        抛 `ValueError`）。`error_code` 默认 `INTERNAL_UNEXPECTED`。
        """
        if exc is None:
            exc = sys.exc_info()[1]
        if exc is None:
    ...
    def tool_call(self, tool: str, cmd: str, exit: int, dur: int, /, **fields: Any) -> None:
        ...
        不接受 `module`，故此处直接构造 TOOL 条目，保证 `module` 正确。
        """
        ...
        self._backend._write_entry(
            entry, error_code=error_code, tid=tid, ctx=self._merged_ctx(fields)
        )
    ...
    def timer(
        ...
        * 成功：按 :func:`classify_duration` 落 INFO/WARN/ERROR（超预算 →
          `error_code=DTO_PERF_EXCEEDED`）；
        * 异常：ERROR + 轨迹 + `error_code`（默认 `INTERNAL_UNEXPECTED`，可用
          `error_code=` 指定业务码），并**原样抛出**；
        * 预算取 `budget_ms`，缺省时取 `perf_budget_ms(op)`；
        * `module` 缺省为本视图的模块标识；`op` 同时写入 ctx 供聚合分组。
        """
        ...
        msg = op if level == "INFO" else f"{op} budget breached ({dur}ms/{budget}ms)"
        self._emit(level, msg, module=module, dur=dur, error_code=code, fields={"op": op, **fields})
...
def get_logger(module: str, **ctx: Any) -> ModuleLogger:
    """模块级 logger 工厂（`program="agenticspec"`，`command` 由模块前缀推导）。

    *rid* 自动注入：每行日志读取 ContextVar 中的请求 rid（见 `rid` 模块）。
    """
    directory = log_dir()
    command = command_for(module)
    key = (str(directory), command)
    with _BACKENDS_LOCK:
        backend = _BACKENDS.get(key)
        if backend is None:
            backend = _FamilyLogger(command, directory)

...
插入/删除其他块不漂移；源文档改版重解析时按 (章节号路径, slug) 匹配 + 人工确认迁移。

单节点原语 :func:`make_anchor` 要求调用方提供「是否为重复组」等事实；整档批量请用
...
SLUG_MAX_CHARS = 80
"""标题 slug 的字符上限；超长时截断并追加标题哈希以保持幂等且可区分。"""

EMPTY_SLUG = "sec"
"""空标题的占位 slug（锚仍需可用且稳定）。"""
...
def slugify(title: str) -> str:
    ...
    CJK 等非 ASCII 字母数字原样保留（不做音译），因此中文标题可得可读 slug
    （如 ``3.1 术语与定义`` → ``术语与定义``）。
    """
    ...
    if len(slug) > SLUG_MAX_CHARS:
        # 截断会让不同标题折叠为同一 slug → 追加标题哈希前缀恢复可区分性（仍幂等）
        slug = f"{slug[:SLUG_MAX_CHARS]}-{hashlib.sha256(norm.encode('utf-8')).hexdigest()[:_TITLE_HASH_LEN]}"
    return slug
...
def normalize_body(text: str) -> str:
    ...
    仅用于摘要计算，不改写库内 ``content``（P4 零改写只约束内容本身）。
    """
...
def section_path_str(section_path: Sequence[str] | str | None) -> str:
    """章节号路径 → ``11.2.3`` 形态（片段间的点号已含者可原样传入）。"""
...
def make_anchor(
    ...
    *,
    ...
    §3 M01 使用的别名（``chapter_path`` / ``occurrence_index`` / ``body_digest``）以
    关键字形式同样可用，便于 M03 预计算摘要后复用。

    :param body_text: 节点正文（参与摘要消歧）；与 ``body_digest`` 给其一即可。
    :param sibling_index: 本节点在同 (章节号路径, slug) 组内的 0 基序号。
    :param body_digest: 预计算的正文摘要（sha256 hex）；给出则不再从 ``body_text`` 计算。
    :param duplicate_title: 是否处于「同父同题」重复组；``None`` 时按「传入序号」或
        「无编号标题」推断为真（无编号标题按 ADR-006 恒定带摘要）。
    :param duplicate_body: 重复组内是否存在正文亦相同的同级（规则 ③）。
    """
    ...
    digest = body_digest if body_digest is not None else (None if body_text is None else body_digest_of(body_text))
    unnumbered = not section_path_str(path)
    duplicated = bool(duplicate_title) if duplicate_title is not None else (index is not None or unnumbered)
    base = anchor_base(doc_id, path, title)
    if not duplicated and not duplicate_body:
        return base
    if digest is None:
        raise ValueError(
            "锚消歧需要正文：提供 body_text 或 body_digest（A1：同父同题必须按正文摘要区分）"
        )
...
def body_digest_of(text: str) -> str:
    """``body_digest`` 的显式调用别名（避免与 :func:`make_anchor` 同名参数混淆）。"""
    return body_digest(text)


@dataclass(frozen=True)
class SectionRef:
    """待定锚的节点最小输入（M03 解析器的输出形态之一）。"""

    section_path: tuple[str, ...] = ()
    """章节号路径路径片段；无编号标题传空 tuple。"""
...
def assign_anchors(doc_id: str, sections: Sequence[SectionRef]) -> list[str]:
    """整档批量定锚：自助发现重复组、同级序号与残余冲突，返回与入参等长的锚列表。

    与 :func:`make_anchor` 的分工：本函数是 M03 的入口（口径单点），
    分组键 = (章节号路径, slug(title))——即锚基底的实际冲突单元（比原始标题更严），
    组内再按正文摘要分「正文亦相同」子组并编同级序号（输入序 = 同级序，稳定）。

    残余冲突（如摘要前缀碰撞、超长标题截断后同基底）按摘要长度阶梯
    （8→12→16→32→64 hex）升级，最后退化为追加输入序号；升级仅依赖文档内容与
    输入顺序，故同一源文档重复解析逐字节一致（幂等）。
    """
    bases: list[str] = []
    digests: list[str] = []
    duplicated: list[bool] = []
    body_index: list[int] = []
    body_duplicated: list[bool] = []

    groups: dict[tuple[str, str], list[int]] = {}
    for pos, section in enumerate(sections):
        path = section_path_str(section.section_path)
        key = (path, slugify(section.title))
        groups.setdefault(key, []).append(pos)
        bases.append(anchor_base(doc_id, path, section.title))
        digests.append(body_digest(section.body_text))

    seen_digest: dict[tuple[str, str], int] = {}
    digest_count: dict[tuple[str, str], int] = {}
    for (path, slug), positions in groups.items():
        for pos in positions:
            dkey = (f"{path}{_SEPARATOR}{slug}", digests[pos])
            digest_count[dkey] = digest_count.get(dkey, 0) + 1
    for (path, slug), positions in groups.items():
        for pos in positions:
            dkey = (f"{path}{_SEPARATOR}{slug}", digests[pos])
            body_index.append(seen_digest.get(dkey, 0))
            seen_digest[dkey] = body_index[-1] + 1
            body_duplicated.append(digest_count[dkey] > 1)

    anchors: list[str] = []
    taken: set[str] = set()
    for pos, section in enumerate(sections):
        positions = groups[(section_path_str(section.section_path), slugify(section.title))]
        in_group = positions.index(pos)
        candidate = make_anchor(
            doc_id,
            section.section_path,
            section.title,
            sibling_index=in_group,
            body_digest=digests[pos],
            duplicate_title=len(positions) > 1,
            duplicate_body=body_duplicated[pos],
        )
        if body_duplicated[pos]:
            candidate = f"{bases[pos]}{_SUFFIX}{digests[pos][:DIGEST_LEN]}{_SUFFIX}{body_index[pos]}"
...
def _escalate(base: str, digest: str, taken: set[str], ordinal: int) -> str:
    """残余冲突升级：加长摘要前缀，最后退化为输入序号（均确定性、幂等）。"""

...
__all__ = ["build_model", "field_deltas", "jsonable", "now", "row_to_dict"]
...
def field_deltas(
    ...
    *,
    ...
    - `create`：`field_deltas({}, row)` → before 全为 `None`；
    - `delete`：`field_deltas(row, {})` → after 全为 `None`，即「before 全量」；
...
def _unused(*_: Sequence[Any]) -> None:  # pragma: no cover - 保持 timezone 导入语义
    del _

...
（:func:`agenticspec.m09.validate_write` / `validate_proposal`）判定**（P5：判据单点，
...
from agenticspec.m09 import validate_proposal, validate_write
...
from agenticspec.store import Storage, ValidationError, get_storage
...
def check_proposals(
    ...
    **判据单点（P5）**：`atom_type` 注册/schema/`content.text`（含量与 `derive_text` 的
    一致性）/锚形态/`doc_type` 组合规则**一律由 M09A 判定**（:mod:`agenticspec.m09`），
    本函数不重复实现，只做两件事：

...
.. code-block:: text
    ...
    role_permits(user.role, perm)                       # (1) 角色硬上限
      AND ( target is None                              # (2) 无目标 → 角色作用域
            OR narrowing(user, perm) 为空               #     该权限没有任何 grant → 角色作用域
            OR ∃ g ∈ narrowing(user, perm).covers(target) )   #     收窄：必须落在 grant 范围内

``narrowing(user, perm)`` = 该用户**覆盖请求权限**的 grant 集合（按 :data:`PERMISSION_IMPLIES`
判定覆盖关系）。**按权限分别收窄**是刻意的：只读 grant（``read``）只收窄读，不会连带
锁死写/审批维度（``grant_matches`` 的 ``perm`` 形参即此语义）。
...
def validate_permission(perm: str) -> str:
    ...
    if perm not in PERMISSIONS:
        raise ValidationError(
            f"unknown permission {perm!r}; expected one of {list(PERMISSIONS)}",
            entity="auth",
        )
    return perm
...
def grant_target_matches(
    ...
    *,
    ...
    return False  # pragma: no cover - DDL CHECK 已限制取值域
...
def applicable_grants(
    ...
    """收窄集：覆盖请求权限的 grant（该权限维度上用户被限定在哪些范围内）。"""
...
def grant_matches(
    ...
    *,
    ...
    ``grants`` 由调用方提供（通常是 :func:`agenticdocer.auth.users.load_grants` 的结果）；
    省略时退化为「无从判定」→ ``False``（fail-closed）。
    """
    validate_permission(perm)
    candidates = applicable_grants(grants or user_grants(user), perm)
    return any(grant_target_matches(g, target, doc_type=doc_type) for g in candidates)


def user_grants(user: User) -> tuple[Grant, ...]:
    """``User`` 行本身不含 grant（独立表）；此处返回空集（fail-closed 的显式表达）。"""
    return ()
...
def validate_grant(role: RoleName, scope: str, value: str, permission: str) -> None:
    ...
    if scope not in GRANT_SCOPES:
        raise ValidationError(
            f"unknown grant scope {scope!r}; expected one of {list(GRANT_SCOPES)}",
            entity="auth",
        )
    if permission not in GRANT_PERMISSIONS:
        raise ValidationError(
            f"grant permission must be one of {list(GRANT_PERMISSIONS)} (S5: 无 admin，"
            f"不可经 grant 提权)；got {permission!r}",
            entity="auth",
        )
...
def targets_equal(left: GrantTarget, right: GrantTarget) -> bool:
    """目标等价（同 kind 同 value）——供调用方去重/比较。"""
    return left.kind == right.kind and left.value == right.value


def doc_type_target(value: str) -> DocTypeTarget:
    """构造 ``doc_type`` 目标（调用方可用它表达「按类型授权」）。"""
    return DocTypeTarget(value=value)


def doc_target(value: str) -> DocTarget:
    """构造 ``doc`` 目标（``doc_type`` 经 ``authorize(doc_type=...)`` 单独传入）。"""
    return DocTarget(value=value)


__all__ += ["doc_target", "doc_type_target", "targets_equal", "user_grants"]
